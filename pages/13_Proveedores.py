...
with tab_mod_masiva:
    st.write("Edita múltiples proveedores y presiona Guardar modificación masiva.")
    if proveedores.empty:
        st.info("No hay proveedores registrados.")
    else:
        ...
        edited_mass = st.data_editor(
            ...
            hide_index=True,
            disabled=["id_proveedor"],
            key="proveedores_modificacion_masiva_editor",
            column_config={"activo": st.column_config.CheckboxColumn("activo")},
        )
        if st.button("Guardar modificación masiva", type="primary", use_container_width=True, key="proveedores_modificacion_masiva_guardar"):
            try:
                for _, row in edited_mass.iterrows():
                    estado = clean_upper(row.get("estado")) or "ACTIVO"
                    if estado not in ESTADOS:
                        raise ValueError(f"Proveedor {row.get('ruc')}: estado inválido {estado}")
                    correo = clean_text(row.get("correo"))
                    if not _is_valid_email(correo):
                        raise ValueError(f"Proveedor {row.get('ruc')}: correo inválido")
                    update_proveedor(
                        int(row["id_proveedor"]), row.get("ruc"), row.get("razon_social"), row.get("rubro_proveedor"),
                        row.get("contacto"), row.get("nro_telefono"), correo, row.get("direccion"), row.get("pais"),
                        row.get("ciudad"), estado, int(bool(row.get("activo"))),
                    )
                st.session_state["msg_proveedor"] = f"Se actualizaron {len(edited_mass)} proveedores correctamente."
                load_proveedores_data.clear()
                st.rerun()
            except Exception as exc:
                st.error("No se pudo guardar la modificación masiva de proveedores.")
                st.exception(exc)
