...
def _ensure_text(statement):
    """Acepta un string SQL o un objeto text() ya construido.
​
    Evita el error de envolver dos veces con text() (TypeError en
    _bind_params_regex.sub cuando se le pasa un TextClause en vez de str).
    """
    ...
    if isinstance(statement, TextClause):
        return statement
    raise TypeError(
        f"El SQL debe ser str o text(), no {type(statement).__name__}"
    )
​
​
...
def execute_statement(statement, params: dict | None = None) -> None:
    """Ejecuta un INSERT/UPDATE/DELETE y confirma la transaccion."""
    # engine.begin() abre una transaccion y hace commit automatico al salir.
    engine = get_engine()
...
def get_stock_general():
    return read_dataframe(
        'SELECT * FROM vw_stock_general ORDER BY nombre_producto'
    )
​
​
def get_stock_por_ubicacion():
    return read_dataframe(
        'SELECT * FROM vw_stock_por_ubicacion '
...
def get_stock_por_cuenta():
    return read_dataframe(
        'SELECT * FROM vw_stock_por_cuenta '
...
def get_movimientos():
    return read_dataframe(
        'SELECT * FROM vw_movimientos ORDER BY fecha_movimiento DESC'
    )
​
​
# ---------------------------------------------------------------------------
...
def insert_producto(
    ...
    query = '''
        INSERT INTO productos
            (sku, nombre, descripcion, id_categoria, id_unidad,
...
​
​
def insert_ubicacion(codigo_ubicacion, id_zona, tipo_ubicacion, pasillo, rack,
                     nivel, posicion, capacidad_maxima):
    query = '''
...
def insert_cuenta(codigo_cuenta, nombre_cuenta, responsable, centro_costo):
    query = '''
        INSERT INTO cuentas_logisticas
            (codigo_cuenta, nombre_cuenta, responsable, centro_costo)
